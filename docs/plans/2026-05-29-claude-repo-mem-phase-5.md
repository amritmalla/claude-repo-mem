# claude-repo-mem Phase 5 — Out-of-scope sweep: Pluggable Embedders, Queue Wiring, Benchmark Harness, Distill UX — Implementation Plan
...
**Embedders**
- `src/claude_repo_mem/embeddings/openai_emb.py` — OpenAI embedder (`text-embedding-3-small` 1536-dim default)
- `src/claude_repo_mem/embeddings/voyage_emb.py` — Voyage (`voyage-3-lite` 512-dim default)
- `src/claude_repo_mem/embeddings/factory.py` — `make_embedder(name)` selecting by env / arg
- Modify: `src/claude_repo_mem/db/schema.py` — `embedder_meta` table; `unit_vec` dim becomes a constructed string fed by the chosen embedder
- Modify: `src/claude_repo_mem/db/connection.py` — `init_db(path, dim=384)` accepts dim
- Modify: `src/claude_repo_mem/cli.py` — `index --embedder NAME` flag; `index --reset` clears + reinits

**Queue wiring**
- Modify: `src/claude_repo_mem/summarizer/backfill.py` — split into `backfill_summaries_sync` (existing) + `enqueue_backfill(settings, llm, queue)`
- Modify: `src/claude_repo_mem/indexer/orchestrator.py` — accept an optional `queue` arg; on full reindex, push a backfill job onto it

**Benchmark harness**
- `src/claude_repo_mem/bench/__init__.py` (empty)
- `src/claude_repo_mem/bench/runner.py` — runs queries from a YAML fixture; computes recall@k
- `src/claude_repo_mem/bench/fixtures/flask_recall_queries.yaml` — 8-12 queries against the Phase 1 flask fixture
- Modify: `src/claude_repo_mem/cli.py` — `claude-repo-mem bench --fixture PATH` subcommand

**Distill UX**
- Modify: `src/claude_repo_mem/distill/extract.py` — `Proposal.dedupe_key()` helper
- Modify: `src/claude_repo_mem/distill/confirm.py` — group proposals by scope, dedupe near-identical fact text before display
...
Today's flow: `claude-repo-mem distill` and explicit `await backfill_summaries(...)` calls. Watcher reindex doesn't run backfill at all.
...
**Files:**
- Modify: `src/claude_repo_mem/db/schema.py`
- Modify: `src/claude_repo_mem/db/connection.py`
...
**Files:**
- Modify: `src/claude_repo_mem/embeddings/base.py` — add `name` attribute
- Modify: `src/claude_repo_mem/embeddings/bge_small.py` — implement `name = "bge-small"`
- Create: `src/claude_repo_mem/embeddings/factory.py`
...
from claude_repo_mem.embeddings.factory import make_embedder


def test_default_is_bge_small(monkeypatch):
    monkeypatch.delenv("CLAUDE_REPO_MEM_EMBEDDER", raising=False)
    e = make_embedder()
...
def test_explicit_bge_small(monkeypatch):
    monkeypatch.setenv("CLAUDE_REPO_MEM_EMBEDDER", "bge-small")
    assert make_embedder().name == "bge-small"


def test_unknown_value_raises(monkeypatch):
    monkeypatch.setenv("CLAUDE_REPO_MEM_EMBEDDER", "bogus")
    with pytest.raises(ValueError):
        make_embedder()


def test_openai_missing_key_raises(monkeypatch):
    monkeypatch.setenv("CLAUDE_REPO_MEM_EMBEDDER", "openai")
...
**Files:**
- Create: `src/claude_repo_mem/embeddings/openai_emb.py`
...
from claude_repo_mem.embeddings.openai_emb import OpenAiEmbedder
...
def test_name_and_dim(env):
    with patch("claude_repo_mem.embeddings.openai_emb.OpenAI") as Client:
        e = OpenAiEmbedder()
...
def test_embed_empty_returns_empty(env):
    with patch("claude_repo_mem.embeddings.openai_emb.OpenAI"):
...
def test_missing_package_raises(monkeypatch):
    ...
    with patch.dict("sys.modules", {"openai": None}):
        with pytest.raises(ImportError):
            ...
            from claude_repo_mem.embeddings import openai_emb
            reload(openai_emb)
...
**Files:**
- Create: `src/claude_repo_mem/embeddings/voyage_emb.py`
...
**Files:**
- Modify: `src/claude_repo_mem/indexer/orchestrator.py` — after a successful reindex with embedder, upsert `embedder_meta(name, dim)`; on entry, refuse if a meta row exists with a different name+dim.
...
if embedder is not None:
    ...
    if row and (row["name"] != embedder.name or row["dim"] != embedder.dim):
        raise ValueError(
            ...
            f"got {embedder.name}/{embedder.dim}. Run `claude-repo-mem index --reset`."
        )
```
...
**Files:**
- Modify: `src/claude_repo_mem/summarizer/backfill.py` — add `backfill_summaries_sync` + `enqueue_backfill`
...
from claude_repo_mem.summarizer.backfill import enqueue_backfill, backfill_summaries_sync
    ...
    def reindex_job():
        ...
        if llm is not None:
            ...
            try:
                ...
                print(f"[claude-repo-mem watcher] backfill failed: {e}", file=sys.stderr)
...
**Files:**
- Create: `src/claude_repo_mem/bench/__init__.py` (empty)
- Create: `src/claude_repo_mem/bench/runner.py`
...
## Task 9: `claude-repo-mem bench` CLI
...
```
git commit -m "feat(cli): claude-repo-mem bench --fixture YAML"
```
...
**Files:**
- Modify: `src/claude_repo_mem/distill/extract.py` — add `Proposal.dedupe_key()`
- Modify: `src/claude_repo_mem/distill/confirm.py` — apply `dedupe_proposals()` and group by scope
...
```
claude-repo-mem index --embedder openai     # OpenAI embeddings (1536d)
claude-repo-mem index --embedder voyage     # Voyage embeddings (512d)
claude-repo-mem index --reset               # nuke DB before reindex (required after embedder switch)
claude-repo-mem bench --fixture queries.yaml --k 5
```
