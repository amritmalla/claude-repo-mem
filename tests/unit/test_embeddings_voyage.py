...
def test_name_and_dim(env):
    with patch("claude_repo_mem.embeddings.voyage_emb.voyageai"):
        from claude_repo_mem.embeddings.voyage_emb import VoyageEmbedder
        e = VoyageEmbedder()
...
def test_embed_returns_float_vectors(env):
    with patch("claude_repo_mem.embeddings.voyage_emb.voyageai") as vy:
